...
- 手机端训练中心 hero 已收紧为紧凑入口卡；四个模块卡统一为同一 `ModuleCard` 移动规格，保持两列网格、同高卡片、对齐标题/说明、右侧插画和更小的圆形箭头。
...
- 登录/注册弹窗当前为蓝白圆角 modal：标题/说明、已学习/已掌握统计卡、登录/注册分段控件、邮箱和验证码输入框、发送/完成验证码主按钮。逻辑仍走 `AuthPanel` 和 `workerAuth` 的邮箱验证码流程。
